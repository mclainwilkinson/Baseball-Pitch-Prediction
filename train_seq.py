...
import numpy as np
import pandas as pd
from model.net_seq import PitchRNN
from model.data_loader_seq import PitchDataset, split_dataset
...
data_file = 'baseballSeq.h5'
data = PitchDataset(data_file)
batch_size = 10
train_split = 0.7
train_loader, test_loader = split_dataset(data, batch_size, 1 - train_split, True, 42)
...
# define the model
model = PitchRNN(input_size=15, output_size=3, hidden_dim=8, n_layers=7)
model.to(device)
print('model loaded')
...
for epoch in range(num_epochs):
    avg_loss = []
    for i, (pitch, seq_len, label) in enumerate(train_loader):
        pitches = Variable(pitch).to(device)
        seq_lens = Variable(seq_len).to(device)
        labels = Variable(label).to(device)

        optimizer.zero_grad()
        out, hidden = model(pitches, seq_lens)
...
loss_df.to_csv('lossSEQ.csv', index=False)

torch.save(model.state_dict(), 'pitchpredSEQ.pkl')
print('model saved to pitchpredSEQ.pkl')
...
# get results
for pitch, seq_len, label in test_loader:
    pitches = Variable(pitch).to(device)
    seq_lens = Variable(seq_len).to(device)
    labels = Variable(label).to(device)
...
confusion.to_csv('confusionSEQweighted.csv', index=False)
